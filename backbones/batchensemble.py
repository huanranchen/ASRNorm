            ...
            if self.random_sign_init:
                if self.probability == -1:
                    ...
                    with torch.no_grad():
                        ...
                        factor2 = torch.Tensor([-1 for i in range(self.num_models - positives_num)])
        ...
        if self.affine:
            for l in self.batch_norms:
                ...
                if self.constant_init:
                    ...
                    nn.init.normal_(l.weight, mean=1.0, std=0.1)
                    # nn.init.normal_(l.weight, mean=1., std=1.)
                    if self.random_sign_init:
                        with torch.no_grad():
                            weight_coeff = torch.randint_like(l.weight, low=0, high=2)
                            weight_coeff.mul_(2).add_(-1)
                            l.weight *= weight_coeff

    def forward(self, input):
        inputs = torch.chunk(input, self.num_models, dim=0)
        res = torch.cat([l(inputs[i]) for i, l in enumerate(self.batch_norms)], dim=0)
        return res
...
class Ensemble_BatchNorm_fast(nn.Module):
    def __init__(
        ...
        if self.track_running_stats:
            self.register_buffer("running_mean", torch.zeros(num_models, num_features))
            self.register_buffer("running_var", torch.ones(num_models, num_features))
        else:
    ...
    def reset_parameters(self):
        ...
        if self.affine:
            ...
            if self.constant_init:
                ...
                if self.random_sign_init:
                    with torch.no_grad():
                        weight_coeff = torch.randint_like(self.weight, low=0, high=2)
                        weight_coeff.mul_(2).add_(-1)
    ...
    def forward(self, input):
        if self.training:
            ...
            input = torch.stack(torch.chunk(input, self.num_models, dim=0), dim=0)
            input = input.transpose(1, 2).reshape(self.num_models, self.num_features, -1)
...
class Ensemble_Conv2dBatchNorm(nn.Module):
    ...
    def reset_parameters(self):
        ...
        # nn.init.normal_(self.gamma, mean=1., std=1)
        if self.bias is not None:
            fan_in, _ = nn.init._calculate_fan_in_and_fan_out(self.conv.conv.weight)
            bound = 1 / math.sqrt(fan_in)
            nn.init.uniform_(self.bias, -bound, bound)
...
class Ensemble_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.training:
            return F.log_softmax(x.view([self.num_models, -1, 10]).mean(dim=0), dim=1)
...
class Ensemble_convNet(nn.Module):
    def __init__(self, num_models):
        ...
        self.conv1 = Ensemble_Conv2d(1, 20, 5, 1, first_layer=True, num_models=num_models)
        self.conv2 = Ensemble_Conv2d(20, 50, 5, 1, first_layer=False, num_models=num_models)
