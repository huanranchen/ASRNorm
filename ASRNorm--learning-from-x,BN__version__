...
class ASRNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        real_mean = x.mean(0)
        real_var = x.std(0)
        asr_mean = self.standard_mean_decoder(F.relu(self.standard_encoder(x)))
        asr_var = F.relu(self.standard_var_decoder(F.relu(self.standard_encoder(x))))
        mean = lambda_1 * asr_mean + (1 - lambda_1) * real_mean
        var = lambda_2 * asr_var + (1 - lambda_2) * real_var

        x = (x - mean) / (var + self.eps)

        asr_mean = torch.tanh(self.rescale_mean_decoder(F.relu(self.rescale_encoder(x)))) + self.bias_1
        asr_var = torch.sigmoid(
            self.rescale_var_decoder(F.relu(self.rescale_encoder(x)))) + self.bias_2
