...
class ASRNormRealBN(nn.Module):
    def __init__(self, dim, eps=1e-6):
        '''

        :param dim: C of N,C,H,D
        '''
        super(ASRNormRealBN, self).__init__()
        self.eps = eps
    ...
    def forward(self, x):
        ...
        lambda_2 = torch.sigmoid(self.lambda_2)
        N, C, H, D = x.shape
        x = x.permute(2, 3, 0, 1)  # H, D, N, C
        real_mean = x.mean(2)  # H, D, C
        real_var = x.std(2)
        asr_mean = self.standard_mean_decoder(
            F.relu(self.standard_encoder(self.drop_out(real_mean.view(H * D, C))))).view(H, D, C)
        asr_var = F.relu(
            self.standard_var_decoder(F.relu(self.standard_encoder(
                self.drop_out(real_var.view(H * D, C)))))).view(H, D, C)
        mean = lambda_1 * asr_mean + (1 - lambda_1) * real_mean
        var = lambda_2 * asr_var + (1 - lambda_2) * real_var

        x = (x - mean.view(H, D, 1, C)) / (var.view(H, D, 1, C) + self.eps)

        asr_mean = torch.tanh(self.rescale_mean_decoder(
            F.relu(self.rescale_encoder(self.drop_out(real_mean.view(H * D, C)))))).view(H, D, C) + self.bias_1
        asr_var = torch.sigmoid(
            self.rescale_var_decoder(
                F.relu(self.rescale_encoder(self.drop_out(real_var.view(H * D, C)))))).view(H, D, C) + self.bias_2
        x = x * asr_var.view(H, D, 1, C) + asr_mean.view(H, D, 1, C)
        x = x.permute(2, 3, 0, 1)  # H, D, N, C
        return x
